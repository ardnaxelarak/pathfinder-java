...
public class CharacterDisplay extends JPanel implements EncounterListener, CharacterListener
{
    ...
    private void drawCharacter(Graphics2D g, Character c, int x, int y)
    {
        ...
        Helper.drawAlignedString(g, fm, c.getName(), x + 5, y + 3, TextLayout.TOP_LEFT);
        if (c.getInitiativeRoll() > Integer.MIN_VALUE)
        {
            String init = String.format("%d (%s)", c.getInitiativeRoll(), Helper.modifierString(c.getInitiativeModifier()));
            Helper.drawAlignedString(g, fm, init, x + WIDTH - 5, y + 3, TextLayout.TOP_RIGHT);
        }
        String hp = String.format("[%d / %d]", c.getCurrentHP(), c.getMaxHP());
        Helper.drawAlignedString(g, fm, hp, x + WIDTH - 5, y + HEIGHT - 3, TextLayout.BOTTOM_RIGHT);
        String ac = String.format("AC: %d", c.getTemplate().getAC());
        Helper.drawAlignedString(g, fm, ac, x + 5, y + HEIGHT - 3, TextLayout.BOTTOM_LEFT);
        if (c.getStatus() != Status.NORMAL)
        {
            Helper.drawAlignedString(g, fm, c.getStatus().toString(), x + 5, y + HEIGHT / 2, TextLayout.CENTER_LEFT);
        }
    }
